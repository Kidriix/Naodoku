...
lines_logos = [f"naodoku\\images\\indices de ligne_svg_picto ligne {i}.svg" for i in range(1, 6)]
...
def bravo():
  st.balloons()
  st.session_state.clickable_ids = clickable_ids
  game_duration = time.gmtime(st.session_state.game_duration)
  st.markdown(f'''
  Grille complétée en **{game_duration.tm_min} min** et **{game_duration.tm_sec} s** bravo !
...
# # --- Lecture du clic précédent ---
# clicked_id = st.query_params.get("cell")
# if clicked_id is not None:
#     col_row = grid_ids_to_letters[int(clicked_id)]
#     select_station(col_row[0], col_row[1])
#     st.query_params.clear()  # évite de re-déclencher au prochain rerun

# --- Couleurs par type de case ---
color_naodoku_bg     = "#b9f2c4"  # case titre NAODOKU
color_naodoku_text   = "#1a5c2e"
color_criteria      = "#eef0f4"  # critères ligne/colonne
color_criteria_text = "#333"
color_criteria_border = "#000000"
color_empty         = "#FFFFFF"  # cases à remplir (déjà votre couleur actuelle)
color_empty_text    = "#000000"
color_empty_border = "#2E7D32"
color_filled        = "#b9f2c4"  # cases remplies par le joueur
...
# --- CSS : force la grille à ne jamais s'empiler + style des cellules ---
st.markdown(f"""
<style>
:root {{
...
# --- Génération de la grille : 4 lignes de 4 colonnes ---
for row in range(5):
